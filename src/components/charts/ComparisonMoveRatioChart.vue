...
const averageValue = computed(() => {
  const moveRatios = props.kpis
    .map(k => k.robots?.task_distribution?.ratio?.MOVE ?? k.performance?.task_ratios?.move_ratio)
    .filter(v => typeof v === "number" && !isNaN(v) && v >= 0 && v <= 1);
  
  if (moveRatios.length === 0) return null;
  const avg = moveRatios.reduce((a, b) => a + b, 0) / moveRatios.length;
...
const chartData = computed(() => {
  ...
  moveRatios.forEach(ratio => {
    const binIndex = Math.min(Math.floor((ratio - min) / binSize), bins - 1);
    binCounts[binIndex]++;
  });
