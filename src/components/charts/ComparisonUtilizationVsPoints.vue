<template>
  <v-card class="kpi-chart-card">
    <v-card-title>Machine Utilization vs. Points</v-card-title>
    <v-card-text>
...
const chartData = computed(() => {
  const dataPoints = props.kpis
    .map(k => ({
      x: k.machines?.by_count?.utilization ?? k.machines?.by_duration?.utilization ?? 0,
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          return `Utilization: ${point.x.toFixed(3)}, Points: ${point.y}`;
        }
      }
    }
  },
