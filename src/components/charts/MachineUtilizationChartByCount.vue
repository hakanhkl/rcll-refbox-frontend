...
const comparisonValue = computed(() => {
  const currentValue = props.kpiData?.machines?.by_count?.utilization ?? 
...
const chartData = computed(() => {
  ...
  if (!stateCounts) return { labels: [], datasets: [] }

  const labels = Object.keys(stateCounts)
  const data = Object.values(stateCounts) as number[]
