<template>
  <v-card class="kpi-chart-card">
    <v-card-title>Movement Availability vs. Points</v-card-title>
    <v-card-text>
  ...
  const dataPoints = props.kpis
    .map(k => {
      ...
      if (typeof k.robots?.movement_availability?.mean === "number") {
        movementAvail = k.robots.movement_availability.mean;
      } else if (Array.isArray(k.robots?.movement_availability?.robots) && 
        ...
        movementAvail = sum / k.robots.movement_availability.robots.length;
      }
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          return `Movement Availability: ${point.x.toFixed(3)}, Points: ${point.y}`;
        }
      }
    }
  },
