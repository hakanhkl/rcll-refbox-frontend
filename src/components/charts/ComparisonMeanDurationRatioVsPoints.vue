<template>
  <v-card class="kpi-chart-card">
    <v-card-title>Mean Duration Ratio to Global vs. Points</v-card-title>
    <v-card-text>
...
const chartData = computed(() => {
  const dataPoints = props.kpis
    .map(k => ({
      x: k.orders?.summary?.mean_duration_ratio_to_global ?? 
         k.orders?.summary?.mean_duration_ratio ?? 
         k.performance?.mean_duration_ratio_to_global ?? 0,
...
const chartOptions = {
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          return `Ratio: ${point.x.toFixed(3)}, Points: ${point.y}`;
        }
      }
    }
  },
  scales: {
    x: { 
      ...
      title: {
        display: true,
        text: "Mean Duration Ratio to Global"
      }
    },
    y: {
      beginAtZero: true,
