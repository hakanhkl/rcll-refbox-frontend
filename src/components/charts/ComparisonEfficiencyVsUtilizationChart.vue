<template>
  <v-card class="kpi-chart-card">
    <v-card-title>Overall Efficiency vs. Points</v-card-title>
    <v-card-text>
...
function calculateLinearRegression(dataPoints: { x: number; y: number }[]) {
  ...
  // Finde min und max x-Werte für die Linie
...
const chartData = computed(() => {
  const dataPoints = props.kpis
    .map(k => ({
      x: k.performance?.overall_efficiency,
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          return `Efficiency: ${point.x.toFixed(3)}, Points: ${point.y}`;
        }
      }
    }
  },
