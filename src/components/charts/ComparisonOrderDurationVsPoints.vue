<template>
  <v-card class="kpi-chart-card">
    <v-card-title>Mean Order Duration vs. Points</v-card-title>
    <v-card-text>
...
const chartData = computed(() => {
  const dataPoints = props.kpis
    .map(k => ({
      x: k.orders?.summary?.mean_duration_seconds ?? 0,
...
const chartOptions = {
    ...
    tooltip: {
      callbacks: {
        label: (context: any) => {
          ...
          return `Duration: ${point.x.toFixed(2)}s, Points: ${point.y}`;
        }
      }
    }
  },
  scales: {
    x: { 
      ...
      title: {
        display: true,
        text: "Mean Order Duration (seconds)"
      }
    },
    y: {
      beginAtZero: true,
