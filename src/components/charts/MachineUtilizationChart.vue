...
const comparisonValue = computed(() => {
  ...
  const currentValue = props.kpiData?.machines?.by_duration?.utilization ?? 
                       props.kpiData?.machines?.by_count?.utilization ?? 
...
const chartData = computed(() => {
  ...
  // Try by_duration.state_durations first, fallback to by_count.state_counts
  const stateDurations = m?.by_duration?.state_durations
  const stateCounts = m?.by_count?.state_counts
  
  if (!stateDurations && !stateCounts) return { labels: [], datasets: [] }

  // Prefer durations over counts for better representation
  const stateData = stateDurations || stateCounts
  const labels = Object.keys(stateData)
  const data = Object.values(stateData) as number[]
